...
const facadePath = path.join(root, `${narrativeRoot}/theory-narrative.mapper.ts`);
const assemblyPath = path.join(root, `${mappingRoot}/theory-document-assembly.ts`);
const facade = fs.readFileSync(facadePath, 'utf8');
const assembly = fs.readFileSync(assemblyPath, 'utf8');

for (const contract of ['buildTheoryNarrativeDocument', 'buildTheoryDocument']) {
  if (!facade.includes(contract)) throw new Error(`Narrative facade lost published contract: ${contract}`);
}
if (!facade.includes('assembleTheoryDocument')) {
  throw new Error('Narrative facade no longer delegates to the document assembly owner');
}
if (!assembly.includes('export function assembleTheoryDocument')) {
  throw new Error('Document assembly owner lost its internal assembly contract');
}
for (const contract of ['buildMacroPages', 'buildScopedPages']) {
  if (!assembly.includes(contract)) throw new Error(`Document assembly lost page owner dependency: ${contract}`);
}
if (facade.includes('buildTheoryDocumentAssembly')) {
  throw new Error('Phantom internal contract leaked into the public narrative facade');
}
...
console.log('PASS SO-009 Narrative God Slayer Closeout: public facade and internal assembly contracts are correctly owned, bounded and regression-armored.');
