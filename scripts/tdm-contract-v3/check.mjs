...
requireCondition(contract.activePhase === 'hardening-release', 'fase hardening-release não está ativa');
requireCondition(contract.version === '3.3.0', 'versão do contrato não corresponde ao hardening');
...
for (const relativePath of forbiddenPaths) {
  requireCondition(!exists(relativePath), `código temporário ainda existe: ${relativePath}`);
}
...
if (exists('src/app/canvas/page.tsx')) {
  ...
  requireCondition(canvasPage.includes("from './canvas-workspace'"), '/canvas não importa CanvasWorkspace diretamente');
...
requireCondition(
  exists('src/app/canvas/canvas-workspace.module.sass') && sha256('src/app/canvas/canvas-workspace.module.sass') === protectedSassHash,
...
for (const forbidden of ['localStorage', 'sessionStorage', 'window.location', 'resend-command-preview-v2', 'canvas-v4']) {
  requireCondition(!officialSource.includes(forbidden), `referência proibida detectada: ${forbidden}`);
}

const packageJson = JSON.parse(read('package.json'));
for (const scriptName of ['typecheck', 'test:unit', 'test:e2e', 'check:tdm:ownership', 'check:tdm:v3:quick', 'check:tdm:v3:full', 'validate:canvas:release']) {
...
if (mode !== 'quick') {
  try {
    execFileSync(process.execPath, ['scripts/tdm-contract-v3/check-ownership.mjs'], { cwd: root, stdio: 'pipe' });
  } catch {
    requireCondition(false, 'contrato de ownership e colocation falhou');
  }
...
console.log(`PASS: TDM Contract V3 ${mode} hardening fechado e íntegro.`);
