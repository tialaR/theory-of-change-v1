...
const owner=path.join(repo,base,'narrative-mapping/theory-graph-traversal.ts'); const fail=(m)=>{console.error(`FAIL SO-009 Wave 02: ${m}`);process.exit(1)};
if(!fs.existsSync(owner)) fail('graph traversal owner missing'); const mapper=read(mapperPath), source=read(owner);
if(mapper.split(/\r?\n/).length>120) fail('mapper exceeded current facade budget');
for(const token of ['buildNarrativeGraph(','resolveScopedFlow(','enumerateLinearPaths(','findWeaklyConnectedComponents(']) if(mapper.includes(token)) fail(`mapper reclaimed traversal primitive ${token}`);
for(const token of ['export function prepareNarrativeGraph','export function resolveNarrativeEndpoints','export function countNarrativePaths','export function findDisconnectedNarrativeNodes']) if(!source.includes(token)) fail(`owner contract missing ${token}`);
if(!reachable(mapperPath,owner)) fail('graph traversal owner is not reachable from mapper facade');
const scss=noScss(); if(scss.length) fail(`.scss files are forbidden: ${scss.join(', ')}`);
console.log('PASS SO-009 Narrative God Slayer Wave 02: graph traversal ownership remains reachable and armored.');
