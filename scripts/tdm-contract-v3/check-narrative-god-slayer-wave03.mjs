...
const owner=path.join(repo,base,'narrative-mapping/theory-condition-builders.ts'); const fail=(m)=>{console.error(`FAIL SO-009 Wave 03: ${m}`);process.exit(1)};
if(!fs.existsSync(owner)) fail('condition owner missing'); const mapper=read(mapperPath), source=read(owner);
if(mapper.split(/\r?\n/).length>120) fail('mapper exceeded current facade budget');
for(const token of ['function emitRiskCallouts','function emitHypothesisCallouts','riskTransitionSentence(','hypothesisTransitionSentence(','NARRATIVE_LABELS.risk','NARRATIVE_LABELS.hypothesis']) if(mapper.includes(token)) fail(`mapper reclaimed condition responsibility ${token}`);
for(const token of ['export function emitRiskConditions','export function emitHypothesisConditions','appendMarkerCallout',"{ kind: 'edge', id: edgeId }"]) if(!source.includes(token)) fail(`condition owner contract missing ${token}`);
if(!reachable(mapperPath,owner)) fail('condition owner is not reachable from mapper facade through page/domain builders');
const scss=noScss(); if(scss.length) fail(`.scss files are forbidden: ${scss.join(', ')}`);
console.log('PASS SO-009 Narrative God Slayer Wave 03: condition ownership remains reachable and armored.');
