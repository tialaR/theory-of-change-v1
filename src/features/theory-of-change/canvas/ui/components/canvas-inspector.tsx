...
function NodeInspector() {
  ...
  function updateFormField(field: 'title' | 'description' | 'advancedDetails', value: string) {
    ...
    runtime.updateSelectedNode(field, value);
  }
  ...
  function saveForm() {
    ...
    runtime.ui.notify(runtime.t('notices.stageUpdated', { stage: copy.singular }));
  }
  ...
  return (
    <div className={styles.inspectorContent} data-stage={node.data.stage}>
      ...
      <label>
        ...
        <ClearableField value={formValue.title} onClear={() => updateFormField('title', '')} label={runtime.t('node.clearTitle')}>
          <input value={formValue.title} onChange={(event) => updateFormField('title', event.target.value)} placeholder={copy.titlePlaceholder} />
        </ClearableField>
      </label>
      <label>
        {runtime.t('node.description')}
        <ClearableField value={formValue.description} onClear={() => updateFormField('description', '')} label={runtime.t('node.clearDescription')} multiline>
          <textarea value={formValue.description} onChange={(event) => updateFormField('description', event.target.value)} placeholder={copy.descriptionPlaceholder} />
        </ClearableField>
      </label>
      <div className={styles.inspectorAdvanced} data-open={runtime.ui.inspectorAdvancedOpenKey === advancedKey}>
        ...
        {runtime.ui.inspectorAdvancedOpenKey === advancedKey ? (
          <ClearableField value={formValue.advancedDetails} onClear={() => updateFormField('advancedDetails', '')} label={runtime.t('node.clearAdvanced')} multiline>
            <textarea className={styles.advancedField} value={formValue.advancedDetails} onChange={(event) => updateFormField('advancedDetails', event.target.value)} placeholder={copy.advancedPlaceholder} />
          </ClearableField>
        ) : null}
      </div>
      ...
      <button type="button" className={styles.inspectorAction} onClick={saveForm}>
