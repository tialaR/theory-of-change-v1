...
import { GlassSurface, getLiquidGlassStageTheme } from '../liquid-glass';
                ...
                style={
                  {
                    ...
                    '--stage-accent-soft': liquidTheme.accentSoftColor
                  } as CustomStyle
                }
              >
                <span className={styles.diagramColumnHalo} aria-hidden="true" />
                <GlassSurface
                  {...COLUMN_GLASS}
                  className={styles.diagramColumnGlass}
                  contentClassName={styles.diagramColumnContent}
                  style={{ '--glass-accent': meta.accent } as CustomStyle}
                  ariaLabel={`${meta.label}, ${countLabel}`}
                >
                  <header className={styles.diagramColumnHeader}>
                    <span className={styles.diagramColumnTopRule} aria-hidden="true" />
                    <div className={styles.diagramColumnHeadingGroup}>
                      <span className={styles.diagramColumnDot} aria-hidden="true" />
                      <h3>{meta.label}</h3>
                    </div>
                    <small>{countLabel}</small>
                  </header>
                  <span className={styles.diagramColumnDivider} aria-hidden="true" />

                  <div className={styles.diagramColumnCards}>
                  {stageNodes.map((node) => {
                    ...
                    return (
                      ...
                      >
                        <GlassSurface
                          {...CARD_GLASS}
                          className={[
                            styles.diagramCardGlass,
                            isSelected ? styles.diagramCardGlassSelected : '',
                            isRelatedOnly ? styles.diagramCardGlassRelated : ''
                          ]
                            .filter(Boolean)
                            .join(' ')}
                          contentClassName={styles.diagramCardInner}
                          style={{ '--glass-accent': meta.accent } as CustomStyle}
                          ariaLabel={`${node.title}, ${meta.label}`}
                        >
                          <span className={styles.diagramCardHeader}>
                            <span className={styles.diagramCardIcon} aria-hidden="true">
                              <span className={styles.diagramCardDot} />
                            </span>
                            <strong>{node.title}</strong>
                            <span className={styles.diagramCardCounts}>
                              {counts.incoming > 0 ? (
                                <span className={styles.diagramCardCountPill}>
                                  <span className={styles.diagramCardCountArrow} aria-hidden="true">
                                    ←
                                  </span>
                                  <span>{counts.incoming}</span>
                                </span>
                              ) : null}
                              {counts.outgoing > 0 ? (
                                <span className={styles.diagramCardCountPill}>
                                  ...
                                  <span>{counts.outgoing}</span>
                                </span>
                              ) : null}
                            </span>
                          </span>
                          <span className={styles.diagramCardDescription}>{node.description}</span>
                          {node.advancedDetails ? (
                            <span className={styles.diagramCardSection}>
                              <em>DETALHES</em>
                              {node.advancedDetails}
                            </span>
                          ) : null}
