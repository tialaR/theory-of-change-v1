...
const COLOR_BENDS_BAND_WIDTH_SCALE = 42.85;

function resolveBandWidth(bandWidth: number) {
  return bandWidth > 0 && bandWidth < 1 ? bandWidth * COLOR_BENDS_BAND_WIDTH_SCALE : bandWidth;
}
...
export default function ColorBends({
  ...
  const resolvedColors = color ? [color] : colors;
  const resolvedBandWidth = resolveBandWidth(bandWidth);
  ...
  useEffect(() => {
    ...
    const material = new THREE.ShaderMaterial({
      ...
      uniforms: {
        ...
        uBandWidth: { value: resolvedBandWidth }
      },
  ...
  useEffect(() => {
    ...
    material.uniforms.uBandWidth.value = resolvedBandWidth;
    ...
    const arr = (resolvedColors || []).filter(Boolean).slice(0, MAX_COLORS).map(toVec3);
  ...
  const fadeTopMask =
    fadeTop != null
      ? `linear-gradient(180deg, rgba(0, 0, 0, ${fadeTop}) 0%, rgba(0, 0, 0, 1) 26%, rgba(0, 0, 0, 1) 100%)`
      : undefined;
  ...
  const containerStyle: React.CSSProperties = {
    ...
    ...(fadeTopMask
      ? {
          maskImage: fadeTopMask,
          WebkitMaskImage: fadeTopMask
        }
      : {})
  };
