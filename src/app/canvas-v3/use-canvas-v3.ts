...
export type ConnectionDraft = { nodeId: string; side: HandleSide; x: number; y: number };
...
export function useCanvasV3() {
  ...
  function beginConnectionDrag(event: React.PointerEvent<HTMLButtonElement>, nodeId: string, side: HandleSide) {
    event.preventDefault();
    event.stopPropagation();
    const node = nodes.find((item) => item.id === nodeId);
    if (!node) return;
    const anchorX = node.x + (side === 'right' ? NODE_WIDTH : 0);
    const anchorY = node.y + NODE_HEIGHT / 2;
    setConnectionDraft({ nodeId, side, x: anchorX, y: anchorY });
    setConnectSource(nodeId);
    setSelectedId(nodeId);
    setMenuId(null);
    notify('Arraste a conexão até o dot do card de destino.', 'neutral');
  }

  function moveConnectionDraft(event: React.PointerEvent<HTMLDivElement>) {
  ...
  function finishConnectionDrag(event: React.PointerEvent<HTMLButtonElement>, targetId: string, targetSide: HandleSide) {
    event.preventDefault();
    event.stopPropagation();
    const draft = connectionDraft;
    if (!draft) return;
    if (draft.nodeId === targetId) {
      cancelConnection();
      return;
    }

    const first = nodes.find((node) => node.id === draft.nodeId);
    const second = nodes.find((node) => node.id === targetId);
    if (!first || !second) {
      cancelConnection();
      return;
    }
    ...
    let source = first;
    let target = second;
    if (!canConnect(source.stage, target.stage) && canConnect(target.stage, source.stage)) {
      source = second;
      target = first;
    }
    ...
    if (!canConnect(source.stage, target.stage)) {
      ...
      notify(connectionMessage(first.stage, second.stage), 'warning');
      return;
    }

    const expectedStartSide = source.id === first.id ? draft.side : targetSide;
    const expectedEndSide = target.id === second.id ? targetSide : draft.side;
    if (expectedStartSide !== 'right' || expectedEndSide !== 'left') {
      setConnectionDraft(null);
      setConnectSource(null);
      notify('Puxe do dot direito da origem até o dot esquerdo do destino.', 'warning');
      return;
    }
    ...
    notify('Conexão criada. Use a seta para adicionar risco ou hipótese.', 'success');
  }
  ...
  function startConnection(nodeId: string) {
    ...
    setSelectedId(nodeId);
    setMenuId(null);
    notify('Conexões são manuais: arraste o dot direito deste card até o dot esquerdo da próxima etapa.', 'neutral');
  }
  ...
  function deleteNode(nodeId: string) {
    ...
    setSelectedId(null);
    setMenuId(null);
    if (connectSource === nodeId) cancelConnection();
  ...
  return {
    canvasRef, nodes, edges, counts, past, future, savedVersions, selectedId, editingId, menuId, connectSource, connectionDraft, editingEdgeId, notice, zoom, theoryName,
    ...
    createNode, startNodeDrag, moveNode, finishNodeDrag, beginConnectionDrag, moveConnectionDraft, finishConnectionDrag, startConnection, cancelConnection, updateNode, saveNode,
