...
- valor de referência: `clamp(4.5rem, 7vw, 7rem)` em desktop/tablet;
- mobile pode reduzir semanticamente para `clamp(3.5rem, 12vw, 4.5rem)`;
...
- Token: `--tdm-public-section-gap` em `_tdm-public-action.sass` (`clamp(4.5rem, 7vw, 7rem)`; mobile `clamp(3.5rem, 12vw, 4.5rem)` via `@media (max-width: 48rem)` em `_tdm-tokens.sass`).
