...
if (exists(files.workspaceController)) {
  const source = read(files.workspaceController);
  requireCondition(source.includes('useCanvasViewportActions'), 'workspace controller voltou a concentrar viewport e framing');
  requireCondition(!source.includes('getNodesBounds'), 'workspace controller voltou a calcular enquadramento do grafo');
  requireCondition(!source.includes('viewportBeforeInspectorRef'), 'workspace controller voltou a guardar viewport do inspector');
}
