...
export function useCanvasWorkspaceController(initialProject: CanvasProject, userName: string) {
  ...
  const { selectedNode, selectedEdge, selectedEdgeSource, selectedEdgeTarget, selectedRelationKind } = useCanvasSelection({
    nodes: flow.nodes,
    edges: flow.edges,
  ...
  const { startStageDrag, allowStageDrop, dropStage } = useCanvasStageDragAndDrop({
    reactFlow,
  ...
  const { undo, redo } = useCanvasHistoryActions({
    undoFlow: flow.undo,
    redoFlow: flow.redo,
    markDirty,
    notify: ui.notify,
