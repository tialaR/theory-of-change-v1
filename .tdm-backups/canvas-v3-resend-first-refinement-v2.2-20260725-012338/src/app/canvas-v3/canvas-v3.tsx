...
export function CanvasV3() {
  ...
  const [edgeEditorKind, setEdgeEditorKind] = useState<'risk' | 'hypothesis' | null>(null);
    ...
    <main className={styles.page} data-focus={focusMode}>
      ...
      <section className={styles.workspace}>
          ...
          >
              ...
              {canvas.edges.map((edge) => {
                ...
                return (
                  <g key={edge.id} className={styles.edgeGroup} data-active={canvas.editingEdgeId === edge.id}>
                    <path className={styles.edgeHit} d={path} onClick={(event) => { event.stopPropagation(); canvas.setEditingEdgeId(edge.id); setEdgeEditorKind(null); }} />
                    ...
                    <foreignObject x={mx - 14} y={my - 14} width="28" height="28" className={styles.edgeActionWrap}>
                      <button type="button" className={styles.edgeAction} onClick={(event) => { event.stopPropagation(); canvas.setEditingEdgeId(edge.id); setEdgeEditorKind(null); }} title="Adicionar risco ou hipótese">{icons.chevron}</button>
                    </foreignObject>
                  </g>
                );
              })}
            ...
            {canvas.nodes.map((node) => (
              <CanvasV3Node
                ...
                onSelect={() => {
                  if (canvas.connectSource && canvas.connectSource !== node.id) {
                    canvas.completeConnection(node.id);
                    return;
                  }
        ...
        {editingEdge && (() => {
          ...
          const top = (y1 + y2) / 2;
          return (
            <section className={styles.edgeInlineEditor} style={{ left, top }} onClick={(event) => event.stopPropagation()}>
              {!edgeEditorKind ? (
                <>
                  <header><small>Qualificar passagem</small><button type="button" onClick={() => canvas.setEditingEdgeId(null)} title="Fechar">{icons.close}</button></header>
                  <div className={styles.edgeKindPicker}>
                    <button type="button" onClick={() => setEdgeEditorKind('risk')}>{icons.risk}<span>Adicionar risco</span></button>
                    <button type="button" onClick={() => setEdgeEditorKind('hypothesis')}>{icons.hypothesis}<span>Adicionar hipótese</span></button>
                  </div>
                  <button type="button" className={styles.edgeDeleteAction} onClick={() => canvas.deleteEdge(editingEdge.id)}>Excluir conexão</button>
                </>
              ) : (
                <>
                  <header>
                    <div><small>Qualificar passagem</small><strong>{edgeEditorKind === 'risk' ? 'Risco' : 'Hipótese'}</strong></div>
                    <button type="button" onClick={() => setEdgeEditorKind(null)} title="Voltar">{icons.back}</button>
                  </header>
                  <label>
                    <span>{edgeEditorKind === 'risk' ? icons.risk : icons.hypothesis}{edgeEditorKind === 'risk' ? 'O que pode atrapalhar?' : 'O que precisa ser verdadeiro?'}</span>
                    <textarea
                      autoFocus
                      rows={3}
                      value={edgeEditorKind === 'risk' ? editingEdge.risk : editingEdge.hypothesis}
                      placeholder={edgeEditorKind === 'risk' ? 'Descreva um risco desta passagem.' : 'Descreva uma hipótese desta passagem.'}
                      onChange={(event) => canvas.updateEdge(editingEdge.id, edgeEditorKind === 'risk' ? { risk: event.target.value } : { hypothesis: event.target.value })}
                    />
                  </label>
                  <div className={styles.edgeInlineActions}>
                    <button type="button" onClick={() => setEdgeEditorKind(null)}>Cancelar</button>
                    <button type="button" onClick={() => { canvas.setEditingEdgeId(null); setEdgeEditorKind(null); canvas.notify('Passagem atualizada.', 'success'); }}>Salvar</button>
                  </div>
                </>
              )}
            </section>
