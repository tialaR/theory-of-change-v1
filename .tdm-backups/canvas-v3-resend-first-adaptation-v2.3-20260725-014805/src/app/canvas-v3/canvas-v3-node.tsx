...
export function CanvasV3Node(props: Props) {
  ...
  return (
    ...
    >
      <button type="button" data-no-drag className={styles.inputHandle} onClick={(event) => { event.stopPropagation(); props.onInput(); }} aria-label="Receber conexão" title="Receber conexão" />
      <button type="button" data-no-drag className={styles.outputHandle} onClick={(event) => { event.stopPropagation(); props.onOutput(); }} aria-label="Criar conexão" title="Criar conexão" />
