...
import { canConnect, createId, stageMeta, STAGES, type CanvasEdge, type CanvasNode, type Snapshot, type StageId } from './canvas-v3.model';
...
export function useCanvasV3() {
  ...
  function createNode(stage: StageId, x: number, y: number) {
    ...
    notify(`${meta.label} adicionado. Complete o conteúdo no próprio card.`, 'success');
  }
  ...
  function startConnection(nodeId: string) {
    ...
    setMenuId(null);
    notify('Selecione o ponto de entrada da próxima etapa.', 'neutral');
  }

  function completeConnection(targetId: string) {
    if (!connectSource) {
      notify('Escolha primeiro o ponto de saída de um card.', 'warning');
      return;
    }
    ...
    if (!canConnect(source.stage, target.stage)) {
      setConnectSource(null);
      notify(`Passagem incoerente. Conecte ${stageMeta(source.stage).label} à etapa causal seguinte.`, 'warning');
      return;
    }
    ...
    notify('Conexão criada. Use a seta da passagem para adicionar risco ou hipótese.', 'success');
  }
  ...
  function saveVersion() {
    ...
    try {
      localStorage.setItem('tdm-canvas-v3', JSON.stringify(version));
    } catch {}
  ...
  return {
    ...
    createNode, startNodeDrag, moveNode, finishNodeDrag, startConnection, completeConnection, updateNode, saveNode,
