...
export function CanvasV3Node(props: Props) {
  ...
  return (
    ...
    >
      <button type="button" className={styles.inputHandle} onClick={(event) => { event.stopPropagation(); props.onInput(); }} title="Receber conexão" aria-label="Receber conexão" />
      <button type="button" className={styles.outputHandle} onClick={(event) => { event.stopPropagation(); props.onOutput(); }} title="Criar conexão" aria-label="Criar conexão" />

      <div className={styles.nodeMeta}>
        <span>{meta.label}</span>
        <button type="button" className={styles.moreButton} onClick={(event) => { event.stopPropagation(); props.onToggleMenu(); }} title="Ações do card" aria-label="Ações do card">{icons.more}</button>
      </div>
      ...
      {editing ? (
        ...
        <>
          ...
          <div className={styles.nodeFooter}>
            <button type="button" onClick={(event) => { event.stopPropagation(); props.onOutput(); }} title="Conectar à próxima etapa">{icons.connect}<span>Conectar</span></button>
      ...
      {menuOpen && !editing && (
        <div className={styles.nodeMenu} data-no-drag onClick={(event) => event.stopPropagation()}>
          <button type="button" onClick={props.onEdit}>{icons.edit}<span>Editar</span></button>
          <button type="button" onClick={props.onDuplicate}>{icons.duplicate}<span>Duplicar</span></button>
          <button type="button" onClick={props.onOutput}>{icons.connect}<span>Conectar</span></button>
          <button type="button" className={styles.dangerAction} onClick={props.onDelete}>{icons.trash}<span>Excluir</span></button>
        </div>
      )}
    </article>
  );
