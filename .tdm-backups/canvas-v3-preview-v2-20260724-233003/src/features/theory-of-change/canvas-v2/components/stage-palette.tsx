  ...
  return (
    <section className={styles.palette} data-open={open}>
      <button className={styles.paletteTrigger} type="button" onClick={onToggle} aria-expanded={open}>
        ...
        <span><strong>Etapas da teoria</strong><small>Arraste ou clique para adicionar</small></span>
        <i>{icons.chevron}</i>
      </button>
      <div className={styles.stageStrip} aria-label="Resumo das etapas">
        {STAGES.map((stage) => <span key={stage.id} data-stage={stage.id}><i />{stage.singular}<b>{counts[stage.id]}</b></span>)}
      </div>
      {open ? (
